...
/**
 * @description 阴历（农历）日历工具类,公历转农历工具类(1700-3100)
 * @from https://github.com/iceenongli/nongli
 * @author bazhandao
...
public class LunarDateUtils {
    ...
    private static String addLastMonth(String thisYear, String lastYear) {
        ...
        //mid=10
        String mid = getNovemberAndDecember(last);
    ...
    private static int[] cast(String start, String now, int[] bigOrLitter, int leap) {
        ...
        //去年11月1日的相对天数,为负数
        int sum = 0 - bigOrLitterSort[0] - bigOrLitterSort[1] - 29 - 29;
        //月份
        int i = 0;
        while (dif >= sum) {
            // 加上每月的农历天数
            sum += (bigOrLitterSort[i++] + 29);
        }
        //获取年份
        ...
        //计算日期
        result[2] = dif - sum + bigOrLitterSort[i - 1] + 29 + 1;
        return result;
    }

    private static int[] resetSort(int[] bigOrLitter, int leap) {
        //15
        int len = bigOrLitter.length;
        ...
        if(leap == 0){
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (index > leap) {
                    ...
                    // 14
                    bigOrLitterSort[i] = bigOrLitter[len - 1];
                } else {
                    bigOrLitterSort[i] = bigOrLitter[i];
                }
            }
     ...
     * @return
     */
    public static String toLunarDate(LocalDate localDate) {
        return toLunarDate(localDate.toString());
    }
     ...
     * @return
     */
    public static String toLunarDate(Date date) {
        return toLunarDate(DateUtils.toLocalDate(date));
    }

    /**
     * 公历日期转农历日期,公历日期合法性经过检查. 推荐的调用方法
     *
     * @param date 公历日期 yyyy-MM-dd,yyyyMMdd,yyyy年MM月dd日,yyyy.MM.dd
     * @return 农历日期
     */
    public static String toLunarDate(String date) {
    ...
    private static String[] read() {
        ...
        try (BufferedReader br = new BufferedReader(
                new InputStreamReader(LunarDateUtils.class.getClassLoader().getResourceAsStream("config.cal")))) {
            String every;
            ...
            log.error("读取文件出错,file={},{}", "config.cal", e);
        }
        return null;
    }
}
